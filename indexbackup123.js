...
async function recursivelyQueueBlock(blockNumber, increment) {
  ...
  fetch('https://blockscout.com/lukso/l14/api/?module=block&action=eth_block_number', {
    ...
    //  annotateImage('0x6623b3bcef6a8f2328d49283ae15deb460084589','https://ipfs.lukso.network/ipfs/Qmev9TiXXQCCWW8QHX8PwFbqpDjwMQ2eFLyUqrmEeuivqu',99);
                annotateImage(data.result[i].address,'https://ipfs.lukso.network/ipfs/' + x.LSP3Profile.LSP3Profile.profileImage[i].url.substr(7),99);
...
app.post('/', async function(req, res) {
  ...
  const myContracts = await lspFactory.LSP3UniversalProfile.deploy({
    controllingAccounts: [req.body.address], // Account addresses which will control the UP
    lsp3Profile: myUniversalProfileData,
  });
  console.log(myContracts);
  const myContract = new web3.eth.Contract(UniversalProfile.abi, myContracts.ERC725Account.address);
